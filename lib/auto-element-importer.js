...
module.exports = class AutoElementImporter {
  ...
  autoElementImport(scope) {
    this.elementsToImport = this.elementsToImport.filter(
      (pkg) => !!pkg && !scope.excludeElements.includes(pkg.name));
    ...
    scope.projectTree = this.tmpImportsDir;
    scope.htmlImportsFile = path.join(this.tmpImportsDir, 'html-imports.html');
  }
};
