...
module.exports = class Config {
	constructor(app, ui) {
		...
		const addonOptions = app.options['ember-cli-polymer'] || {};
	...
	set bundlerOutput(bundlerOutput) {
		if (path.extname(bundlerOutput) === '.html') {
			...
			throw new Error('[ember-cli-polymer] The `bundlerOutput` file ' +
	...
	set vulcanizeOutput(vulcanizeOutput) {
      ...
      'renamed to `bundlerOutput` and will be removed in 3.x (ember-cli-polymer)');
	...
	set vulcanizeOptions(vulcanizeOptions) {
      ...
      'renamed to `bundlerOptions` and will be removed in 3.x (ember-cli-polymer)');
